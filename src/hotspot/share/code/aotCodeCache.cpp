...
bool AOTCodeCache::finish_write() {
  ...
  if (store_count > 0) {
    ...
    log_info(aot, codecache, exit)("  Shared Blobs:  total=%d", total_shared_blobs_count);
    log_info(aot, codecache, exit)("  C1 Blobs:      total=%d", total_C1_blobs_count);
    log_info(aot, codecache, exit)("  C2 Blobs:      total=%d", total_C2_blobs_count);
...
bool AOTCodeCache::store_code_blob(CodeBlob& blob, AOTCodeEntry::Kind entry_kind, uint id, const char* name, int entry_offset_count, int* entry_offsets) {
  ...
  if (AOTCodeEntry::is_adapter(entry_kind) && !cache->adapter_caching()) {
    return false;
  }
